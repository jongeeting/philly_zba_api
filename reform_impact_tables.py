...
Shows complete elimination vs partial help
"""
...
def would_be_allowed_tier1(issues):
    ...
    for issue_type, value in issues.items():
        ...
        elif issue_type == 'dwelling_units' and value <= 3:
            resolved.append(f'units_{value}')
...
def would_be_allowed_tier2(issues):
    ...
    for issue_type in tier1_remaining:
        if issue_type == 'roof_deck':
            ...
            # Tier 2: Allow up to 6 units
            if issues[issue_type] <= 6:
                resolved.append(f'units_{issues[issue_type]}_tier2')
            else:
                remaining.append(issue_type)
...
# Print Tier 1 table
print("\n" + "="*100)
print("TIER 1 REFORMS IMPACT (4 Stories + Triplex + Parking Elimination)")
print("="*100)
...
# Print Tier 2 table
print("\n" + "="*100)
print("TIER 1 + TIER 2 REFORMS COMBINED (+ Roof Decks + Sixplex + Commercial)")
print("="*100)
...
print("- Tier 1: Height ≤4 stories, Triplex (≤3 units), Parking elimination")
print("- Tier 2: Adds Roof Decks, Sixplex (≤6 units), Commercial/Mixed-Use")
